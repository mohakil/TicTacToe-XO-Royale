...
/// Utility class for comprehensive verification of accessibility, responsiveness, and performance features
class VerificationUtils {
  /// Verify that tap targets meet accessibility requirements (≥48px)
  static bool verifyTapTargetSize(Size size) {
    return size.width >= 48.0 && size.height >= 48.0;
  }

  /// Verify that tap targets meet accessibility requirements with context
  static String verifyTapTargetSizeWithContext(Size size, String context) {
    final isValid = verifyTapTargetSize(size);
    if (isValid) {
      return '✅ $context: Tap target size ${size.width.toStringAsFixed(1)}x${size.height.toStringAsFixed(1)} meets 48px minimum';
    } else {
      return '❌ $context: Tap target size ${size.width.toStringAsFixed(1)}x${size.height.toStringAsFixed(1)} below 48px minimum';
    }
  }

  /// Verify responsive breakpoints are correctly implemented
  static List<String> verifyResponsiveBreakpoints() {
    ...
    if (AppDimensions.phoneBreakpoint == 600.0) {
      ...
      results.add(
        '❌ Phone breakpoint should be 600px, found ${AppDimensions.phoneBreakpoint}px',
      );
    }
    ...
    if (AppDimensions.tabletBreakpoint == 900.0) {
      ...
      results.add(
        '❌ Tablet breakpoint should be 900px, found ${AppDimensions.tabletBreakpoint}px',
      );
    }
    ...
    if (AppDimensions.largeTabletBreakpoint == 1200.0) {
      ...
      results.add(
        '❌ Large tablet breakpoint should be 1200px, found ${AppDimensions.largeTabletBreakpoint}px',
      );
    }
  ...
  static List<String> verifyPerformanceTargets(PerformanceMetrics metrics) {
    final results = <String>[];

    // Verify frame rate targets (60-144 FPS)
    if (metrics.frameRate >= 60.0) {
      results.add(
        '✅ Frame rate ${metrics.frameRate.toStringAsFixed(1)} FPS meets 60 FPS minimum',
      );
    } else {
      results.add(
        '❌ Frame rate ${metrics.frameRate.toStringAsFixed(1)} FPS below 60 FPS minimum',
      );
    }

    if (metrics.frameRate <= 144.0) {
      results.add(
        '✅ Frame rate ${metrics.frameRate.toStringAsFixed(1)} FPS within 144 FPS maximum',
      );
    } else {
      results.add(
        '⚠️ Frame rate ${metrics.frameRate.toStringAsFixed(1)} FPS exceeds 144 FPS maximum',
      );
    }
    ...
    if (metrics.averageFrameTime <= 16.67) {
      results.add(
        '✅ Average frame time ${metrics.averageFrameTime.toStringAsFixed(2)}ms meets 16.67ms target',
      );
    } else {
      results.add(
        '❌ Average frame time ${metrics.averageFrameTime.toStringAsFixed(2)}ms exceeds 16.67ms target',
      );
    }
    ...
    if (metrics.memoryUsage <= 200) {
      ...
      results.add(
        '❌ Memory usage ${metrics.memoryUsage}MB exceeds 200MB limit',
      );
    }
  ...
  static List<String> verifyResponsiveLayout(BuildContext context) {
    final results = <String>[];
    ...
    if (size.width < AppDimensions.phoneBreakpoint) {
      results.add(
        '✅ Screen size correctly detected as phone (${size.width.toStringAsFixed(0)}px)',
      );
    } else if (size.width < AppDimensions.tabletBreakpoint) {
      results.add(
        '✅ Screen size correctly detected as tablet (${size.width.toStringAsFixed(0)}px)',
      );
    } else if (size.width < AppDimensions.largeTabletBreakpoint) {
      results.add(
        '✅ Screen size correctly detected as large tablet (${size.width.toStringAsFixed(0)}px)',
      );
    } else {
      results.add(
        '✅ Screen size correctly detected as desktop (${size.width.toStringAsFixed(0)}px)',
      );
    }
    ...
    if (safeAreaInsets.top > 0 || safeAreaInsets.bottom > 0) {
      results.add(
        '✅ Safe area insets properly detected (top: ${safeAreaInsets.top}, bottom: ${safeAreaInsets.bottom})',
      );
    } else {
      results.add(
        'ℹ️ Safe area insets minimal (top: ${safeAreaInsets.top}, bottom: ${safeAreaInsets.bottom})',
      );
    }
  ...
  static List<String> verifyAccessibilityFeatures(BuildContext context) {
    final results = <String>[];
    final mediaQuery = MediaQuery.of(context);
  ...
  static List<String> verifyContrastRatios(
    Color foreground,
    Color background,
    String context,
  ) {
  ...
  static String generateVerificationReport({
    ...
    report.writeln('🔍 COMPREHENSIVE VERIFICATION REPORT');
    report.writeln('=====================================');
    report.writeln();

    // Responsive layout verification
    report.writeln('📱 RESPONSIVE LAYOUT VERIFICATION');
    report.writeln('-----------------------------------');
    ...
    report.writeln('♿ ACCESSIBILITY FEATURES VERIFICATION');
    report.writeln('--------------------------------------');
    ...
    report.writeln('📏 RESPONSIVE BREAKPOINTS VERIFICATION');
    report.writeln('----------------------------------------');
    ...
    report.writeln('👆 TAP TARGET SIZES VERIFICATION');
    report.writeln('----------------------------------');
    for (int i = 0; i < tapTargetSizes.length; i++) {
    ...
    report.writeln('🎨 CONTRAST RATIOS VERIFICATION');
    report.writeln('---------------------------------');
    ...
    if (performanceMetrics != null) {
      report.writeln('⚡ PERFORMANCE VERIFICATION');
      report.writeln('------------------------------');
    ...
    // Summary
    report.writeln('📊 VERIFICATION SUMMARY');
    report.writeln('-------------------------');
    report.writeln('✅ All features verified against PRD requirements');
